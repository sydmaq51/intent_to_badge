...
if 'auth_status' not in st.session_state or st.session_state.auth_status == 'not_authed': 
  ...
  with st.form("badge_name_and_email"):
    ...
    if submit_edits:
      session.call('AMAZING.APP.UPDATE_BADGENAME_BADGEEMAIL_SP',st.session_state.uni_id, st.session_state.uni_uuid, edited_given.replace("'","''"), edited_middle.replace("'","''"), edited_family.replace("'","''"), edited_email)
